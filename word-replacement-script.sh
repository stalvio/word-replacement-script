...
if [ ! -s "specific_java_files.txt" ]; then
		# File exists, continue with operations
			echo "specific_java_files.txt is empty"
		else
			echo "specific_java_files.txt is not empty"
		fi
echo 
...
while read line || [ -n "$line" ]; do
    ...
    for java_file in "${java_files[@]}"; do
		...
		if [ -f "$java_file" ]; then
		# File exists, continue with operations
			sed -i "s/$source_word/$replacement_word/g" "$java_file"
		else
...
echo "Word replacements completed."